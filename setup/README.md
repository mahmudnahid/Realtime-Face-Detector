...
2. The goal of the setup process is to create a virtual environment and install necessary packages for running the notebooks. The required packages as defined in the `requirements_faced.txt` file will be installed in the environment. There are two ways to accomplish this, so you can choose the method that works best for you:
    ...
    2. **Manually** - following the steps below in the command line:
        ...
        3. `cd` out back to the repository root folder, and activate the virtual environment by running:
            * Linux or Mac: `source ./venv/faced/bin/activate` 
            * Windows CMD: `.\venv\faced\Scripts\activate.bat`
            * Windows Powershell: `.\venv\faced\Scripts\Activate.ps1`
         4. Install the packages: `pip install -r requirements_faced.txt`
         5. Install `ipykernel`: `python -m ipykernel install --user --name=faced --display-name=faced`
...
* Then, open a notebook and set the right kernel(`faced`). Setting the kernel is done on the top right side in `vscode`. 
...
### Testing the Setup
To make sure that the environment, and the `vscode` integration were all set up correctly - open the file `setup/test_setup.ipynb`, set the right kernel(`faced`), and run. If you get a `succeeded` message - these modules are working fine. 